...
// Q: [n_tokens, n_heads * head_dim/2] u32 (packed f16, Phase C3)
// K_cache: [max_seq, kv_dim/2] u32 (packed f16, Phase A — 2 K-values
//          per u32). Semantic kv_dim/head_dim in Params is the f32
//          count; shader does /2 internally for u32 indexing.
...
@group(0) @binding(0) var<storage, read> q: array<u32>;  // packed f16 (C3)
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    // Q and K cache both packed f16 (Phase C3 / A respectively): linear
    // f32 index N → u32 index N/2.
    ...
    let k_u32_base = t * kv_dim_half + kv_h * head_dim_half;

    var dot: f32 = 0.0;
    for (var dp: u32 = 0u; dp < head_dim_half; dp = dp + 1u) {
        let q_pair = unpack2x16float(q[q_u32_base + dp]);
        let k_pair = unpack2x16float(k_cache[k_u32_base + dp]);
