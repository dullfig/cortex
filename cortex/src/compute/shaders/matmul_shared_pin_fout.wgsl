// Phase C1 matmul: PACKED-input, F32-output variant of matmul_shared.
//
// Identical to matmul_shared.wgsl in tile layout, MADD pattern, and
// dispatch shape — only the input buffer type is changed to packed
// f16 (array<u32> with 2 f16 per u32). Used for Q/K/V/O/gate/up
// projections after scratch.normed becomes packed (C1). The down_proj
// still uses original matmul_shared because scratch.activated remains
// f32 in C1 (becomes packed in C2).
...
// Phase C1: input packed f16 (2 per u32).
@group(0) @binding(1) var<storage, read> input_mat: array<u32>;
...
fn main(
    ...
    for (var ks: u32 = 0u; ks < num_k_steps; ks = ks + 1u) {
        ...
        // ---- Cooperative load: A tile (weights, packed f16) ----
        // Same as matmul_shared.wgsl.
        if (lidx < 128u) {
        ...
        // ---- Cooperative load: B tile (input, PACKED f16) ----
        // Each of the first 64 threads loads 2 u32s (= 4 f16) and unpacks.
        if (lidx < 64u) {
        ...
        // ---- Per-thread MADDs (identical to matmul_shared.wgsl) ----
        let li_b: u32 = 16u + li;
